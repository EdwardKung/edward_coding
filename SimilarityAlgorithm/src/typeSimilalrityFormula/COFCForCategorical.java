...
public class COFCForCategorical implements FormulaInterface {
	...
	public Float getDistanceExecute(DataSet input, int targetIndex,
		...
		{
			//System.out.println();
			//System.out.println("A1:P("+A1+"|"+compareSet[j]+")="+A1ConditionEvent.get(compareSet[j])+"/"+A1Set.getDataCount());
			//System.out.println("A2:P("+A2+"|"+compareSet[j]+")="+A2ConditionEvent.get(compareSet[j])+"/"+A2Set.getDataCount());
			result+=Math.abs(((float)A1ConditionEvent.get(compareSet[j])/A1Set.getDataCount())-((float)A2ConditionEvent.get(compareSet[j])/A2Set.getDataCount()));
